...
public class SwingGUI extends JFrame implements UI, IObserver {
	...
	private final JToolBar toolBar = new JToolBar("Still draggable");

	static final private String UNDO = "previous";
	static final private String REDO = "next";

	// Game Stuff
	private final GameController controller = GameController
			.getInstance();;
	private final Player[] players = controller.getPlayers();
	...
	public SwingGUI() {
		...
		final BoxLayout lineAxisLayout = new BoxLayout(lineAxisPanel,
				BoxLayout.LINE_AXIS);
		final BoxLayout pageAxisLayout = new BoxLayout(pageAxisPanel,
				BoxLayout.PAGE_AXIS);
		...
		// pageAxisPanel.setPreferredSize(new Dimension(400, 300));
	...
	public void drawGameField() {
		...
		for (int currentRow = 0; currentRow < GameField.DEFAULT_ROWS; currentRow++) {
			for (int currentColumn = 0; currentColumn < GameField.DEFAULT_COLUMNS; currentColumn++) {
				...
				if (player == null) {
					coinCells[currentRow][currentColumn]
					...
					JOptionPane.showMessageDialog(this,
							"Fehler beim Einf��gen der M��nze in Zeile "
