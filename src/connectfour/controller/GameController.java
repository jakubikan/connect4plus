...
import connectfour.model.gameField.GameFieldEdit;
    ...
    public boolean dropCoinWithSuccessFeedback(final int col) {
        ...
        if (!userHasWon()) {
            try {
                ...
                System.out.println(p.getName() + " making move: " + move);
                ...
                String undoInfo = String.format("Undoing %s Player Move", getPlayerOnTurn()
                                                        .getName());
                GameFieldEdit edit = new GameFieldEdit(previousState, newState, undoInfo);
                undoManager.addEdit(edit);
                System.out.println("Added Undo for " + undoManager.getUndoPresentationName());
